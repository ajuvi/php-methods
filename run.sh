...
if [ "$operacio" = "+" ]; then
	...
	curl "http://localhost/php-rest-calc/rest.php?num1=80&num2=5"

elif [ "$operacio" = "-" ]; then
	#envia a través del mètode HTTP POST
	curl -X POST http://localhost/php-rest-calc/rest.php \
	     -H "Content-Type: application/json" \
	     -d '{"num1": "80", "num2": "5"}'

elif [ "$operacio" = "*" ]; then
	#envia a través del mètode HTTP PUT
        curl -X PUT http://localhost/php-rest-calc/rest.php \
             -H "Content-Type: application/json" \
             -d '{"num1": "80", "num2": "5"}'

elif [ "$operacio" = "/" ]; then
	#envia a través del mètode HTTP DELETE
        curl -X DELETE http://localhost/php-rest-calc/rest.php \
             -H "Content-Type: application/json" \
             -d '{"num1": "80", "num2": "5"}'
