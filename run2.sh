...
if [ "$operacio" = "+" ]; then
        ...
        curl "http://localhost/rest-calc/rest.php?num1=${n1}&num2=${n2}"

elif [ "$operacio" = "-" ]; then
        #envia a través del mètode HTTP POST
        curl -X POST http://localhost/rest-calc/rest.php \
             -H "Content-Type: application/json" \
             -d "{\"num1\": $n1, \"num2\": $n2}"

elif [ "$operacio" = "*" ]; then
        #envia a través del mètode HTTP PUT
        curl -X PUT http://localhost/rest-calc/rest.php \
             -H "Content-Type: application/json" \
             -d "{\"num1\": $n1, \"num2\": $n2}"

elif [ "$operacio" = "/" ]; then
        #envia a través del mètode HTTP DELETE
        curl -X DELETE http://localhost/rest-calc/rest.php \
             -H "Content-Type: application/json" \
             -d "{\"num1\": $n1, \"num2\": $n2}"
